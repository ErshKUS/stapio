...
# file_catalog_json = "/home/ershkus/osmCatalog/catalog.json"
# path_dictionary_json = "/home/ershkus/osmCatalog/dictionary/"
lang=['ru']
fupd_time = 'upd_time_poi.json'
file_log = 'poi.log'


def catalog_BACKUP(cur, isupdate=False, whereTime=None, loglevel=0):
  log.add ('start load catalog', level=loglevel, file=file_log)
  dictionary_json=[]

  with open(conf.file_catalog_json, 'r', encoding='utf-8') as f:
    catalog_json = json.loads(f.read())
  
  for fd in os.listdir(conf.path_dictionary_json):
    with open(conf.path_dictionary_json+fd, 'r', encoding='utf-8') as f:
  ...
  for icat in catalog_json:
    tags = [k+'='+v for k,v in icat['tags'].items()]
    tags.sort()
  ...
  for icatalog in catalog_json:
    wheretags=""
    wheretagsR=""
    for k,v in icatalog['tags'].items():
      if v == '*':
        wheretags=wheretags+("AND tags ? '%(k)s' " % {"k":k})
        wheretagsR=wheretagsR+("AND relations.tags ? '%(k)s' " % {"k":k})
      else:
        wheretags=wheretags+("AND tags @> '%(k)s=>%(v)s' " % {"k":k, "v":v})
        wheretagsR=wheretagsR+("AND relations.tags @> '%(k)s=>%(v)s' " % {"k":k, "v":v})
    if 'tags_ex' in icatalog:
      for k,v in icatalog['tags_ex'].items():
      ...
      for ilang in dictionary_json:
        ...
        for mt in icatalog['moretags']:
          ...
          if (icatalog['moretags'][mt]['type'] == 'translate'):
            case1 = case1 + "CASE "
            for k,v in ilang['class'][icatalog['moretags'][mt]['class']].items():
...
def catalog(cur, isupdate=False, whereTime=None, loglevel=0):
  ...
  if not isupdate:
    log.add ('clear table ershkus_poi_c', level=loglevel+1, file=file_log)
    cur.execute("DELETE FROM ershkus_poi_c")
  ...
  for icatalog in catalog_json:
    ...
    dataIn = {}
    dataIn['cur'] = cur
    dataIn['whereTime'] = whereTime
    dataIn['loglevel'] = loglevel
    dataIn['wheretimeN'] = wheretimeN
    dataIn['wheretimeALL'] = wheretimeALL
    dataIn['langname'] = langname
    dataIn['langinsert'] = langinsert
    dataIn['icatalog'] = icatalog
    dataIn['dictionary_json'] = dictionary_json
    
    dataToThread.append(dataIn)
    
  
  pool = ThreadPool(conf.countThread)
  results = pool.map(catalogInThread, dataToThread)
  pool.close()
  pool.join()
...
def catalogInThread(dataIn):


  # cur = dataIn['cur']
  whereTime = dataIn['whereTime']
  loglevel = dataIn['loglevel']

  wheretimeN = dataIn['wheretimeN']
  wheretimeALL = dataIn['wheretimeALL']
  langname = dataIn['langname']
  langinsert = dataIn['langinsert']
  icatalog = dataIn['icatalog']
  dictionary_json = dataIn['dictionary_json']
...
def addr_BACKUP(cur, lastID=0, loglevel=0):
  log.add ('start set addr', level=loglevel, file=file_log)

  # cur2 = conn2.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
  cur.execute("""SELECT min(id), max(id) FROM ershkus_poi_c""")
  limit = cur.fetchone()
  ...
  for n in range(limit['min'], limit['max'], conf.poi_inAddrStep):
    ...
    cur.execute("""
      UPDATE ershkus_poi_c
      SET
        addr_region_id=ershkus_search_addr.id
      FROM
        ershkus_search_addr_p as ershkus_search_addr
      WHERE
        ershkus_poi_c.id>=%(min)s AND ershkus_poi_c.id<%(max)s
        AND ((ershkus_search_addr.geom && ershkus_poi_c.c_geom) AND ST_Covers(ershkus_search_addr.geom, ershkus_poi_c.c_geom))
        AND ershkus_search_addr.addr_type_id=10
        AND addr_full_name is null
      ;
    """, {'min': n, 'max': n+conf.poi_inAddrStep})
    ...
    cur.execute("""
      ...
      WHERE
        ...
        AND ershkus_poi_c.addr_region_id=ershkus_search_addr.region_id
        AND addr_full_name is null
      ;
    ...
    cur.execute("""
      ...
      WHERE
        ...
        AND ershkus_poi_c.addr_region_id=ershkus_search_addr.region_id
        AND ershkus_poi_c.addr_city_id=ershkus_search_addr.city_id
        AND ershkus_poi_c.addr_house is null
        AND addr_full_name is null
      ;
    """, {'min': n, 'max': n+conf.poi_inAddrStep})
  ...
  cur.execute("""
    ...
    FROM
      ershkus_search_addr_p as ershkus_search_addr
    WHERE
      ((ershkus_search_addr.geom && ershkus_poi_c.c_geom) AND ST_Covers(ershkus_search_addr.geom, ershkus_poi_c.c_geom))
      AND ershkus_search_addr.addr_type_id=25
      AND ershkus_poi_c.addr_region_id=ershkus_search_addr.region_id
      AND ershkus_poi_c.addr_full_name is null
      AND ershkus_poi_c.id >= %(lastID)s;
  """, {"lastID": lastID})
  ...
  cur.execute("""
    ...
    FROM
      ershkus_search_addr_p as ershkus_search_addr
    WHERE
      ershkus_poi_c.addr_region_id=ershkus_search_addr.id
      AND ershkus_search_addr.addr_type_id=10
      AND ershkus_poi_c.addr_full_name is null
      AND ershkus_poi_c.id >= %(lastID)s;
  """, {"lastID": lastID})
  ...
  cur.execute("""
    ...
    SET
      ...
      --index_name = (ershkus_poi_c.class_ru || ' , ' || ershkus_poi_c.name_ru || ' , ' || ershkus_poi_c.addr_full_name)
    FROM
      ershkus_search_addr
    WHERE
      ershkus_poi_c.addr_in_id = ershkus_search_addr.id
...
def addr(cur, lastID=0, loglevel=0):
  ...
  pool = ThreadPool(conf.countThread)
  results = pool.map(addrInThread, dataToThread)
  pool.close()
  pool.join()
